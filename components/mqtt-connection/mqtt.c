...
{
    if (error_code != 0) {
        ESP_LOGE(MQTT_LOG_TAG, "Last error %s: 0x%x", message, error_code);
    }
}
...
{
    // Why did we have to modify the last format parameter from "%d" to "%ld"?
    ESP_LOGD(MQTT_LOG_TAG, "Event dispatched from event loop base=%s, event_id=%ld", base, event_id);
...
esp_mqtt_client_handle_t mqtt_app_start()
{
    ...
    esp_log_level_set(MQTT_LOG_TAG, ESP_LOG_VERBOSE);
    ...
    if (strcmp(mqtt_cfg.broker.address.uri, "FROM_STDIN") == 0) {
        ...
        ESP_LOGE(MQTT_LOG_TAG, "Configuration mismatch: wrong broker url");
        abort();
    }
